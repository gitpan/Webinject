# This Makefile is for the Webinject extension to perl.
#
...
#     ABSTRACT => q[Perl Module for testing web services]
#     AUTHOR => q[Corey Goldberg, <corey@goldb.org>]
#     BUILD_REQUIRES => {  }
#     DISTNAME => q[Webinject]
#     LICENSE => q[gpl2]
#     MIN_PERL_VERSION => q[5.006]
#     NAME => q[Webinject]
#     NO_META => q[1]
#     PREREQ_PM => { Getopt::Long=>q[0], ExtUtils::MakeMaker=>q[6.42], XML::Simple=>q[0], Time::HiRes=>q[0], HTTP::Cookies=>q[0], HTTP::Request::Common=>q[0], Error=>q[0], File::Temp=>q[0], Crypt::SSLeay=>q[0], XML::Parser=>q[0], LWP=>q[0] }
#     VERSION => q[1.50]
#     VERSION_FROM => q[lib/Webinject.pm]
#     dist => { PREOP=>q[$(PERL) -I. "-MModule::Install::Admin" -e "dist_preop(q($(DISTVNAME)))"] }
#     realclean => { FILES=>q[MYMETA.yml] }
#     test => { TESTS=>q[t/01-Webinject.t t/02-Test_Cases.t t/03-Report_Type_Nagios.t t/04-Timeouts.t t/97-Pod.t t/98-Pod-Coverage.t t/99-Perl-Critic.t] }
...
TO_INST_PM = lib/Webinject.pm

PM_TO_BLIB = lib/Webinject.pm \
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=$(MAN3EXT) --perm_rw=$(PERM_RW) \
	  lib/Webinject.pm $(INST_MAN3DIR)/Webinject.$(MAN3EXT) 
...
TEST_FILE = test.pl
TEST_FILES = t/01-Webinject.t t/02-Test_Cases.t t/03-Report_Type_Nagios.t t/04-Timeouts.t t/97-Pod.t t/98-Pod-Coverage.t t/99-Perl-Critic.t
TESTDB_SW = -d
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="$(DISTNAME)" VERSION="1.50">' > $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>Perl Module for testing web services</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Corey Goldberg, &lt;corey@goldb.org&gt;</AUTHOR>' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/Webinject.pm blib/lib/Webinject.pm 
